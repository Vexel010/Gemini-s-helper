...
print("=== Система Gemini's Helper запущена ===")
banned_users = []
all_words_text = """
سكس
طيز
...
yaraktan
"""
bad_words = all_words_text.split()
warnings = 0
while True:
    user_input = input("Введите сообщение: ")
    clean_input = "".join(char for char in user_input.lower() if char.isalnum())
    if any(word in clean_input for word in bad_words):
	warnings += 1
	if warnings < 3
	print(f"[WARNING]: Bad word detected! Attempts left: {3 - warnings}")
	continue
       print("[ACCESS BANNED]: Обнаружен хейт!")
        break
